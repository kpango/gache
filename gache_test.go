...
func TestGetShardID_MaxKeyLengthZero(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			id := getShardID(truncateKey(tt.key, 0))
			if id > mask {
				t.Errorf("getShardID(truncateKey(%q, 0)) = %d, want <= %d (mask)", tt.key, id, mask)
			}
			// Result must be deterministic within the same process.
			id2 := getShardID(truncateKey(tt.key, 0))
			if id != id2 {
				t.Errorf("getShardID(truncateKey(%q, 0)) is not deterministic: got %d then %d", tt.key, id, id2)
			}
		})
	}
}
...
func TestGetShardID_MaxKeyLengthOne(t *testing.T) {
	...
	idA1 := getShardID(truncateKey("abc", 1))
	idA2 := getShardID(truncateKey("axyz", 1))
	if idA1 != idA2 {
	...
	idB := getShardID(truncateKey("bcd", 1))
	if idB > mask {
		t.Errorf("getShardID(truncateKey(%q, 1)) = %d, want <= %d (mask)", "bcd", idB, mask)
	}
	// Verify the result is within [0, mask].
	if idA1 > mask {
		t.Errorf("getShardID(truncateKey(%q, 1)) = %d, want <= %d (mask)", "abc", idA1, mask)
	}
}
...
func TestGetShardID_MaxKeyLengthBetween1And32(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			idLong := getShardID(truncateKey(longKey, tt.kl))
			if idLong > mask {
				t.Errorf("getShardID(truncateKey(longKey, %d)) = %d, want <= %d (mask)", tt.kl, idLong, mask)
			}

			idShort := getShardID(truncateKey(shortKey, tt.kl))
			if idShort > mask {
				t.Errorf("getShardID(truncateKey(shortKey, %d)) = %d, want <= %d (mask)", tt.kl, idShort, mask)
			}
...
func TestGetShardID_MaxKeyLengthOver32(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			id1 := getShardID(truncateKey(key1, tt.kl))
			id2 := getShardID(truncateKey(key2, tt.kl))
			if id1 != id2 {
...
func TestGetShardID_KeyShorterThanMaxKeyLength(t *testing.T) {
	...
	// Should equal getShardID(truncateKey("hi", 2)).
	key := "hi"
	id1 := getShardID(truncateKey(key, 100))
	id2 := getShardID(truncateKey(key, 2))
	if id1 != id2 {
		t.Errorf("short key: getShardID(truncateKey(%q, 100))=%d should equal getShardID(truncateKey(%q, 2))=%d", key, id1, key, id2)
	}

	// Single-byte key with large kl: effective length is 1.
	// Should be within [0, mask] and deterministic.
	singleKey := "x"
	idSingle := getShardID(truncateKey(singleKey, 50))
	if idSingle > mask {
		t.Errorf("getShardID(truncateKey(%q, 50)) = %d, want <= %d", singleKey, idSingle, mask)
	}
	// Determinism check.
	idSingle2 := getShardID(truncateKey(singleKey, 50))
	if idSingle != idSingle2 {
		t.Errorf("getShardID(truncateKey(%q, 50)) is not deterministic: got %d then %d", singleKey, idSingle, idSingle2)
	}
}
...
func TestGetShardID_ResultInRange(t *testing.T) {
	...
	for _, c := range cases {
		id := getShardID(truncateKey(c.key, c.kl))
		if id > mask {
			t.Errorf("getShardID(truncateKey(%q, %d)) = %d exceeds mask %d", c.key, c.kl, id, mask)
		}
	}
}
...
func TestGetShardID_PrefixIsolation(t *testing.T) {
	...
	id1 := getShardID(truncateKey(key1, 16))
	id2 := getShardID(truncateKey(key2, 16))
	if id1 != id2 {
		t.Errorf("prefix isolation failed: getShardID(truncateKey(%q, 16))=%d != getShardID(truncateKey(%q, 16))=%d", key1, id1, key2, id2)
	}
	...
	id3 := getShardID(truncateKey(key3, 50))
	id4 := getShardID(truncateKey(key4, 50))
	if id3 != id4 {
		t.Errorf("prefix isolation (xxh3) failed: getShardID(truncateKey(%q, 50))=%d != getShardID(truncateKey(%q, 50))=%d", key3, id3, key4, id4)
	}
}
