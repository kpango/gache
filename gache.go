...
func newMap[V any]() (m *Map[string, value[V]]) {
	m = new(Map[string, value[V]])
	m.read.Store(&readOnly[string, value[V]]{})
	return m
}
...
func (g *gache[V]) get(key string) (v V, expire int64, ok bool) {
	var val *value[V]
	val, ok = g.shards[getShardID(truncateKey(key, g.maxKeyLength))].LoadPointer(key)
	if !ok {
		return v, 0, false
	}
...
func (g *gache[V]) set(key string, val V, expire int64) {
	...
	shard := g.shards[getShardID(truncateKey(key, g.maxKeyLength))]
